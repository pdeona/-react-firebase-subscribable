...
export default function connectFirestore<P>(mapSnapshotsToProps: MapFirestoreFn, ...injectedRefs: RefToInj<P>[]) {
  ...
  return function enhance(WrappedComponent: ComponentType<P>) {
    function FirestoreConsumer(props: P) {
      ...
      useEffect(() => {
        ...
        injectedRefs.forEach((ref) => {
          if (typeof ref === 'function') fnRefs = fnRefs.concat(ref)
          else injectRef(ref as PlainRefToInj)
        })
